render_server_aspek_spatial <- function(params){
  ...
  output$data_table_apekSpatial <- renderDT({
    ...
    datatable(
      ...
      colnames = c(
        "No",
        "ID",
        'Seberapa jauh rumah Anda dari pusat desa atau kota terdekat? .... KM', 
        'Bagaimana Anda biasanya mengakses fasilitas umum seperti pasar, sekolah, dan rumah sakit di desa ini?', 
        'Apakah transportasi umum cukup memadai di desa ini?', 
        'Bagaimana kondisi infrastruktur jalan di desa ini? ', 
        'Apakah terdapat masalah terkait sanitasi di desa ini, seperti akses ke fasilitas toilet yang memadai atau masalah limbah?', 
  ...
  observeEvent(input$update_aspek_id, {
    ...
    updateNumericInput(
      session,
      'Seberapa.jauh.rumah.Anda.dari.pusat.desa.atau.kota.terdekat.KM.edit',
      value = data$Seberapa.jauh.rumah.Anda.dari.pusat.desa.atau.kota.terdekat.KM
    )	
    updateSelectInput(
      session,
  ...
  observeEvent(input$updateAspekSpatial, {
    ...
    if (params == TRUE){
      ...
      data[data$No == input$selected_id_aspek, ] <- data.frame(
        ...
        Seberapa.jauh.rumah.Anda.dari.pusat.desa.atau.kota.terdekat.KM =
          input$Seberapa.jauh.rumah.Anda.dari.pusat.desa.atau.kota.terdekat.KM.edit, 
        Bagaimana.Anda.biasanya.mengakses.fasilitas.umum.seperti.pasar.sekolah.dan.rumah.sakit.di.desa.ini =
  ...
  output$barChartJarakRumah <- renderPlot({
    data <- read_csv(pathTambahAspekSpatial) %>%
      count(Seberapa.jauh.rumah.Anda.dari.pusat.desa.atau.kota.terdekat.KM)
    
    
    custom_colors <- c("red", "green", "blue", "orange", "purple", "yellow", "pink", "brown", "cyan", "magenta")
    
    set.seed(42) 
    shuffled_colors <- sample(custom_colors, length(unique(data$Seberapa.jauh.rumah.Anda.dari.pusat.desa.atau.kota.terdekat.KM)), replace = TRUE)
    
    ggplot(data, aes(x = factor(Seberapa.jauh.rumah.Anda.dari.pusat.desa.atau.kota.terdekat.KM), y = n, fill = factor(Seberapa.jauh.rumah.Anda.dari.pusat.desa.atau.kota.terdekat.KM))) + 
